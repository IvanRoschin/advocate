...
export default function ClientCasesPageClient({ initialCases }: Props) {
  const cases = initialCases;

  const summary = useMemo(
    () => ({
      total: cases.length,
      active: cases.filter(item => ACTIVE_STATUSES.includes(item.status))
        .length,
  ...
  return (
    <div className="mx-auto flex w-full max-w-7xl flex-col gap-6">
      ...
      <section className="border-border bg-card rounded-[28px] border p-6 shadow-sm">
        ...
        {cases.length === 0 ? (
          <div className="border-border bg-background/60 mt-6 rounded-3xl border p-6">
            <div className="text-primary text-base font-medium">
              Справ поки немає
            </div>
            <p className="text-secondary mt-2 text-sm leading-6">
              Коли для вашого профілю будуть створені справи, вони з’являться
              тут.
            </p>
          </div>
        ) : (
              ...
              >
                ...
                <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                  ...
                  <div className="border-border bg-card rounded-2xl border p-4">
                    ...
                    <div className="text-primary mt-2 text-sm font-medium break-all">
                      {item.id}
                    </div>
                  </div>
                </div>
