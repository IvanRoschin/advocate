...
export type RegisterInput = { name: string; phone: string; email: string };
...
async function issueActivationEmail(user: HydratedDocument<UserDocument>) {
  ...
  const verificationUrl = `${serverEnv.baseUrl}${routes.public.auth.verifyEmail}?token=${tokenDoc.token}&email=${encodeURIComponent(user.email)}`;
...
export async function registerAccountLogic(
  args: RegisterInput
...
export async function resendVerificationLogic(
