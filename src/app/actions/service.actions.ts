...
export async function getServicesPublicList(args?: {
  limit?: number;
...
export async function createService(data: CreateServiceRequestDTO) {
  await dbConnect();
...
export async function updateService(id: string, data: UpdateServiceDTO) {
  ...
  if (data.summary !== undefined) service.summary = data.summary;
  if (data.status !== undefined) service.status = data.status;
  if (data.layout !== undefined) service.layout = data.layout;
  if (data.sections !== undefined) service.sections = data.sections;
  if (data.seoTitle !== undefined) service.seoTitle = data.seoTitle;
